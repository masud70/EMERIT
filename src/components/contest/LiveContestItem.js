...
const LiveContestItem = ({ data, mode }) => {
    ...
    const register = () => {
        const url = CONSTANT.BASE_URL + '/contest/register/' + data.id;
        fetch(url, {
            method: 'GET',
            headers: {
    ...
    useEffect(() => {
        ...
        let interval = setInterval(() => {
            ...
            setItem(pre => ({
                ...pre,
                remaining: prettyMilliseconds(calcRemaining(data.start, data.duration, state), {
    ...
    return (
        <View className="rounded overflow-hidden my-1">
            ...
            <View className="w-full bg-gray-600 p-1">
                <Text className="font-bold text-xl text-gray-50">{data.title}</Text>
            </View>
            <View className="bg-green-100 p-1">
                ...
                <View className="w-full flex flex-row">
                    ...
                    <View className="w-4/6">
                        <Text className="text-base text-gray-600">: {data.duration} minutes</Text>
                    </View>
                </View>
                <View className="w-full flex flex-row">
                    <View className="w-2/6">
                ...
                <View className="w-full flex flex-row justify-around">
                    <TouchableOpacity className="bg-green-500 px-4 rounded min-w-[145px] items-center">
                        <Text className="font-bold text-lg text-white">View Details</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        onPress={() => {
                            state === 'Live'
                ...
                {state === 'Ended' && (
                    <View className="w-full items-center my-1">
                        <TouchableOpacity className="bg-green-500 px-4 rounded min-w-[140px] items-center w-11/12">
                            <Text className="font-bold text-lg text-white">View Leaderboard</Text>
                        </TouchableOpacity>
                    </View>
                )}
            </View>
            <View className="bg-gray-200 py-1 px-2 flex flex-row items-center justify-between divide-x-2 divide-gray-400">
                <View className="w-1/2 flex flex-row justify-center items-center">
                    ...
                    <Text>{data.regCount}</Text>
                </View>
                <View className="w-1/2 items-center">
                    <Text>@{data.username || data.User?.username}</Text>
                </View>
            </View>
        </View>
    );
